"""
Skin detection module using HSV color space thresholding and morphological cleaning.
...
class SkinDetector:
    ...
    def _validate_frame_input(self, frame_bgr: np.ndarray) -> None:
        """Validate input BGR frame before calling OpenCV operations."""
        if frame_bgr is None:
            raise TypeError("frame_bgr cannot be None")
        if not isinstance(frame_bgr, np.ndarray):
            raise TypeError(f"frame_bgr must be a numpy.ndarray, got {type(frame_bgr).__name__}")
        if frame_bgr.size == 0:
            raise ValueError("frame_bgr cannot be empty")
        if frame_bgr.ndim != 3:
            raise ValueError(f"frame_bgr must have 3 dimensions (height, width, channels), got {frame_bgr.ndim}")
        if frame_bgr.shape[2] != 3:
            raise ValueError(f"frame_bgr must have exactly 3 color channels, got {frame_bgr.shape[2]}")
        if frame_bgr.shape[0] == 0 or frame_bgr.shape[1] == 0:
            raise ValueError(f"frame_bgr dimensions must be non-zero, got shape {frame_bgr.shape}")
    ...
    def detect_skin(self, frame_bgr: np.ndarray) -> np.ndarray:
        ...
        """
        self._validate_frame_input(frame_bgr)
    ...
    def _apply_hsv_range(
        ...
        low_h, low_s, low_v = int(lower_arr[0]), int(lower_arr[1]), int(lower_arr[2])
        up_h, up_s, up_v = int(upper_arr[0]), int(upper_arr[1]), int(upper_arr[2])
        ...
        if low_h > up_h:
            mask1 = cv2.inRange(
                ...
                np.array([180, up_s, up_v], dtype=np.uint8),
            )
            mask2 = cv2.inRange(
                frame_hsv,
                np.array([0, low_s, low_v], dtype=np.uint8),
                np.array([up_h, up_s, up_v], dtype=np.uint8),
            )
    ...
    def apply_morphology(self, mask: np.ndarray, operation: str = "both") -> np.ndarray:
        ...
        if mask.size == 0 or operation == "none" or operation is None:
            return mask
    ...
    def process_frame(self, frame_bgr: np.ndarray, morphology_op: str = "both") -> SkinDetectionResult:
        """Process BGR frame and return detailed SkinDetectionResult."""
        mask_raw = self.detect_skin(frame_bgr)
        mask_clean = self.apply_morphology(mask_raw, operation=morphology_op)

        raw_pixels = int(np.count_nonzero(mask_raw))
        clean_pixels = int(np.count_nonzero(mask_clean))
        total_pixels = float(frame_bgr.shape[0] * frame_bgr.shape[1])

        return SkinDetectionResult(
            mask_raw=mask_raw,
            mask_clean=mask_clean,
            raw_pixel_count=raw_pixels,
            clean_pixel_count=clean_pixels,
            raw_percentage=(100.0 * raw_pixels / total_pixels) if total_pixels > 0 else 0.0,
            clean_percentage=(100.0 * clean_pixels / total_pixels) if total_pixels > 0 else 0.0,
        )
    ...
    def extract_hsv_from_region(
        ...
        low_h, low_s, low_v = int(lower_raw[0]), int(lower_raw[1]), int(lower_raw[2])
        up_h, up_s, up_v = int(upper_raw[0]), int(upper_raw[1]), int(upper_raw[2])
    ...
    def calibrate_from_samples(
        sample_frames: List[np.ndarray], morph_kernel_size: int = 5
    ...
    def is_valid_hsv_range(lower: np.ndarray, upper: np.ndarray) -> bool:
        ...
        # Value lower bound check: V >= 30 (reject very dark pixels)
        if low_v < 30:
            return False
        ...
        if low_h <= up_h:
            ...
            # Requires low_h >= 120 and up_h <= 60
            if low_h < 120 or up_h > 60:
                return False
            hue_span = (180 - low_h) + up_h

        # Reject excessively broad hue ranges (maximum circular hue span: 110 degrees)
        if hue_span > 110:
            return False
